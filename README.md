...
```
0x23 Di_draw_triangle <- <8> [x1y1: uint16_t, x2y2: uint16_t, x3y3: uint16_t, value: uint8_t, is_fill: uint8_t]
  -> [void]
  
  WHERE `x1y1`:
    0x0000-0xFFFF: coordinates of the A point of the triangle drawn
    
  WHERE `x2y2`:
    0x0000-0xFFFF: coordinates of the B point of the triangle drawn
    
  WHERE `x3y3`:
    0x0000-0xFFFF: coordinates of the C point of the triangle drawn
    
  WHERE `value`:
    0x00-0xFF: gray-scale intensity of the triangle drawn
    
  WHERE `is_fill`:
    0x00: this is a stroke draw request
    0x01: this is a fill draw request
    
    undefined values are treated like 0x01
```

```
0x24 Di_draw_rect <- <6> [x1y1: uint16_t, x2y2: uint16_t, value: uint8_t, is_fill: uint8_t]
  -> [void]
  
  WHERE `x1y1`:
    0x0000-0xFFFF: coordinates of the top-left corner of the rectangle drawn
    
  WHERE `x2y2`:
    0x0000-0xFFFF: coordinates of the bottom-right curner of the rectangle drawn
    
  WHERE `value`:
    0x00-0xFF: gray-scale intensity of the rectangle drawn
...
```
0x25 Di_draw_arc <- <8> [x0y0: uint16_t, radius: uint8_t, start_angle: uint16_t, end_angle: uint16_t, value: uint8_t, is_fill: uint8_t]
  -> [void]
  
  WHERE `x0y0`:
    0x0000-0xFFFF: coordinates of the center point of the arc drawn
    
  WHERE `radius`:
    0x00-0xFF: pixel radius of the arc drawn  
  
  WHERE `start_angle`:
    0x0000-0x0168: angle, in degrees, at which the arc drawn begins
                   values out of range are clamped
    
  WHERE `end_angle`:
    0x0000-0x0168: angle, in degrees, at which the arc drawn ends
                   values out of range are clamped 
                   
  WHERE `value`:
    0x00-0xFF: gray-scale intensity of the arc drawn
...
```
0x26 Di_draw_ellipse <- <6> [x0y0: uint16_t, h_radius: uint8_t, v_radius: uint8_t, value: uint8_t, is_fill: uint8_t]
  -> [void]
  
  WHERE `x0y0`:
    0x0000-0xFFFF: coordinates of the center point of the ellipse drawn
    
  WHERE `h_radius`:
    0x00-0xFF: horizontal pixel radius of the ellipse drawn
    
  WHERE `v_radius`:
    0x00-0xFF: vertical pixel radius of the ellipse drawn
    
  WHERE `value`:
    0x00-0xFF: gray-scale intensity of the ellipse drawn
